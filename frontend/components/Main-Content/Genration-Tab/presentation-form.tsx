...
const dataSourceOptions = [
  ...
  { id: "uploaded_plus_corpus", label: "Uploaded + Database", condition: (files: File[]) => files.length > 0 },
  { id: "corpus_only", label: "From Database Only", condition: () => true },
];


export function PresentationForm({ onGenerate, isLoading , localFiles,setLocalFiles }: PresentationFormProps) {
  ...
  useEffect(() => {
    if (localFiles.length > 0) {
      // Auto select "uploaded_only"
      dispatch(setDataSourceChoice("uploaded_only"));
    } else {
      // No files → select corpus only
      dispatch(setDataSourceChoice("corpus_only"));
    }
  }, [localFiles, dispatch]);
    ...
    switch (step) {
      case 0:
        return dataSourceOptions.some(
          opt => opt.id === dataSourceChoice && opt.condition(localFiles)
        );
      ...
      case 3: // Style
        return style !== ""
      case 4: // Content
        return outline && outline.slides && outline.slides.length > 0;
      case 5: // template
        return template !== ""
      case 6: // Review
        return true
    ...
    if (currentStep === 0) {
      ...
      if (localFiles.length > 0) {
        try {
          ...
          const res = await uploadFilesToBackend(localFiles, (p) => setUploadProgress(p), username || "guest_user");
      ...
      if (
        localFiles.length === 0 && dataSourceChoice === "corpus_only" ||
        localFiles.length > 0 && dataSourceChoice !== ""
      ) {
        setCompletedSteps((prev) => new Set(prev).add(currentStep));
      ...
      // Step 3 (select style) → Step 4 (generate outline)
      if (currentStep === 3) {
        ...
        try {
          ...
          if (result.status === "success" && Array.isArray(result.slides)) {
            dispatch(setOutline(result));
            return;
          }

          setOutlineError(
            result.detail ||
            "No relevant content found. Please change your query and try again."
          );

          dispatch(setOutline(null));  
        }
        catch (err: any) {
          console.error("Outline error:", err);
          setOutlineError("Error generating outline. Please modify your query and try again.");
          dispatch(setOutline(null));  
        }
        finally {
        ...
        try {
          ...
          if (template === "custom_upload" && customTemplateFile) {
            ...
            if (res.status === "generated" && res.presentation_id) {
              dispatch(setPresentationId(res.presentation_id)); //store generated presentation ID
              setCompletedSteps((prev) => new Set(prev).add(currentStep));
              dispatch(setStep(currentStep + 1)); // move to next step
            } else {
          ...
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="mt-6">
              ...
              <select
                ...
                onChange={(e) => dispatch(setDataSourceChoice(e.target.value))}
              ...
              >
                {[
                  ...
                  { id: "uploaded_plus_corpus", label: "Uploaded + Database" },
                  { id: "corpus_only", label: "From Database Only" },
                ].map((opt) => {
                  const isDisabled = (() => {
                    if (localFiles.length > 0) {
                      // Files uploaded → disable corpus only
                      return opt.id === "corpus_only";
                    } else {
                      // No files → disable uploaded options
                      return opt.id === "uploaded_only" || opt.id === "uploaded_plus_corpus";
                    }
                  })();
                  return (
                    <option
              ...
              {localFiles.length === 0 && (
                <p className="text-xs text-muted-foreground mt-1">
                  ⚙️ Only <b>From Database Only</b> is active. Upload files to unlock other options.
                </p>
              )}
            </div>
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="space-y-4">
              ...
              <div className="relative pt-12 pb-6">
                {/* Track Background */}
                <div className="relative h-2 bg-input rounded-full">
                  {/* Active Range Highlight */}
                  <div
                    className="absolute h-full bg-primary rounded-full transition-all"
                    style={{
                      left: `${((minSlides - 5) / 15) * 100}%`,
                      width: `${((maxSlides - minSlides) / 15) * 100}%`
                    }}
                  />
                </div>

                {/* Slider Container */}
                <div className="absolute top-12 left-0 right-0 h-2 -translate-y-1/2">
                  {/* Min Slider */}
                  <input
                    type="range"
                    min="5"
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const value = Number.parseInt(e.target.value)
                      if (value > minSlides) {
                        dispatch(setSlideRange({ min: minSlides, max: value }));
                      }
                    }}
                    className="absolute w-full h-2 bg-transparent appearance-none cursor-pointer range-slider"
                    style={{ zIndex: 4 }}
                ...
                {/* Value Labels */}
                <div
                  className="absolute -top-2 bg-primary text-primary-foreground px-3 py-1 rounded text-xs font-semibold shadow-lg transition-all"
                  style={{ left: `${((minSlides - 5) / 15) * 100}%`, transform: 'translateX(-50%)' }}
                >
                  {minSlides}
                </div>
                <div
                  className="absolute -top-2 bg-primary text-primary-foreground px-3 py-1 rounded text-xs font-semibold shadow-lg transition-all"
                  style={{ left: `${((maxSlides - 5) / 15) * 100}%`, transform: 'translateX(-50%)' }}
                >
                  {maxSlides}
                </div>
              </div>
              ...
              {/* Total Range Display */}
              <p className="text-sm text-muted-foreground">Total range: {maxSlides - minSlides + 1} slides</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const value = Number.parseInt(e.target.value)
                      if (value >= 5 && value < maxSlides) dispatch(setSlideRange({ min: value, max: maxSlides }))
                    }}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const value = Number.parseInt(e.target.value)
                      if (value > minSlides && value <= 20) dispatch(setSlideRange({ min: minSlides, max: value }))
                    }}
                    className="w-full px-3 py-2 border border-input rounded-lg bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary"
                  />
                </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={() => {
                    dispatch(setSlideRange({ min: 5, max: 10 }));
                  }}
                ...
                <button
                  onClick={() => {
                    dispatch(setSlideRange({ min: 10, max: 20 }));
                  }}
                  className={`px-4 py-3 rounded-lg border-2 text-sm font-medium transition-all ${minSlides === 10 && maxSlides === 20
                    ? "border-primary bg-secondary text-primary"
                    : "border-input bg-background text-foreground hover:border-primary/50"
                    }`}
                >
                ...
                <button
                  onClick={() => {
                    dispatch(setSlideRange({ min: 15, max: 20 }));
                  }}
                ...
                <button
                  onClick={() => {
                    dispatch(setSlideRange({ min: 12, max: 18 }));
                  }}
    ...
    `}</style>
        ...
        {currentStep === 3 && (
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { id: "corporate", name: "Professional Corporate", icon: "🏢" },
                { id: "technical", name: "Technical Detailed", icon: "😊" },
                { id: "executive", name: "Executive Summary", icon: "⚡" },
                { id: "sales", name: "Sales and Marketing", icon: "📊" },
              ].map((option) => (
                <button
                  key={option.id}
                  onClick={() => dispatch(setStyle(option.id))}
        ...
        {currentStep === 4 && (
          <div className="space-y-4 animate-fade-in">
            ...
            {loadingOutline ? (
              ...
              <p className="text-sm text-muted-foreground">No outline data received.Try changing Query to get relevent outline</p>
            )}
          </div>
        )}
        {/* Step 5: template */}
                        ...
                        >
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              dispatch(setTemplate("custom_upload"));
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="rounded-2xl border border-blue-200/50 bg-gradient-to-br from-blue-50/80 via-white/60 to-blue-100/60 dark:from-zinc-800/50 dark:via-zinc-900/50 dark:to-zinc-800/50 shadow-lg backdrop-blur-md p-6 space-y-4">
              {[
                ...
                { label: "Files", value: `${localFiles.length} file(s)` },
      ...
      <div className="flex gap-3 justify-between">
        ...
        {currentStep < STEPS.length - 1 ? (
          ...
          >
            {isLoading ? "Generated" : "Generate Presentation"}
          </Button>
        )}
      </div>
    </div>
