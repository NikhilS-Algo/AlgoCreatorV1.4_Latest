...
const Chatbox = () => {
  ...
  const slowMessageTipIntervalRef = useRef<NodeJS.Timeout | null>(null);
  ...
  useEffect(() => {
    ...
    if (last?.from === "chatbot" && last?.isLoading) {
      ...
      const timer = setTimeout(() => {
        ...
        slowMessageTipIntervalRef.current = tipInterval;

      }, 10000);

      return () => {
        clearTimeout(timer);
        if (slowMessageTipIntervalRef.current) {
          clearInterval(slowMessageTipIntervalRef.current);
        }
      };
                      ...
                      >
                        {chat.isLoading ? (
                          <div className="flex flex-col gap-2">
                            ...
                            {/* Always visible: 3 dots + generating text */}
                            ...
                            {/* Visible only after 10 sec */}
                            {showSlowMessage && (
                              <span className="text-sm text-blue-600 italic animate-pulse">
                                {currentTip}
                              </span>
                            )}
