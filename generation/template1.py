...
#                 # Simple split by periods for demonstration
...
#         print("🎨 Creating Dynamic Layout Presentation")
...
#             # Skip the first slide if it's the introduction (like first code)
...
#         print(f"✅ Dynamic Layout Presentation created with {total_content_slides + 1} total slides!")
...
#         """Modern title slide with geometric elements"""
...
#         """Layout with only text content (centered)"""
#         slide = self.prs.slides.add_slide(self.prs.slide_layouts[6])
...
#         """Single image on LEFT side, text on RIGHT"""
#         slide = self.prs.slides.add_slide(self.prs.slide_layouts[6])
...
#         # Image Panel (LEFT) - 45% width
#         image_panel_width = self.slide_width * 0.45
...
#         image_panel.fill.fore_color.rgb = RGBColor(25, 25, 25)
#         image_panel.line.fill.background()
        
#         # Text Panel (RIGHT) - 50% width
#         text_panel_width = self.slide_width * 0.5
#         text_panel_left = Inches(0.5) + image_panel_width + Inches(0.3)
...
#         image_width = image_panel_width - Inches(1.0)
...
#                 height=image_height
#             )
#             self.add_image_caption(slide, image_data, image_left, image_top + image_height + Inches(0.1))
#         except Exception as e:
#             print(f"🖼️ Image error: {e}")
#             self.add_image_placeholder(slide, image_left, image_top, image_width, image_height)
        
#         # Add text content to RIGHT panel
#         text_box = slide.shapes.add_textbox(
#             text_panel_left + Inches(0.4), Inches(1.8),
#             text_panel_width - Inches(0.8), Inches(4.2)
#         )
#         text_frame = text_box.text_frame
#         text_frame.word_wrap = True
...
#         """Double images on RIGHT side, text on LEFT"""
#         slide = self.prs.slides.add_slide(self.prs.slide_layouts[6])
...
#         # Images Panel (RIGHT) - 45% width with two image slots
#         images_panel_width = self.slide_width * 0.45
#         images_panel_left = Inches(0.5) + text_panel_width + Inches(0.3)
#         images_panel = slide.shapes.add_shape(
#             MSO_SHAPE.RECTANGLE,
#             images_panel_left, Inches(1.3),
#             images_panel_width, Inches(5.2)
#         )
#         images_panel.fill.solid()
#         images_panel.fill.fore_color.rgb = RGBColor(25, 25, 25)
#         images_panel.line.fill.background()
        
#         # Add text content to LEFT panel
#         text_box = slide.shapes.add_textbox(
...
#                 height=image_height
#             )
#             self.add_image_caption(slide, image_data_list[0], image_left, image1_top + image_height + Inches(0.05))
#         except Exception as e:
...
#         image2_top = image1_top + image_height + Inches(0.6)
#         try:
...
#                 height=image_height
#             )
#             self.add_image_caption(slide, image_data_list[1], image_left, image2_top + image_height + Inches(0.05))
#         except Exception as e:
...
#     def add_image_caption(self, slide, image_data, left, top):
#         """Add caption below image"""
#         caption = image_data.get("caption", "Related Visual")
#         caption_box = slide.shapes.add_textbox(
#             left, top, Inches(4.0), Inches(0.3)
#         )
...
# # USAGE FUNCTION (same as in app.py)
# def generate_dynamic_presentation(json_file_path, output_path):
#     """Main function to generate presentation - same interface as app.py"""
#     generator = DynamicLayoutPresentationGenerator(json_file_path)
#     generator.create_presentation(output_path)
#     return output_path

# def adapt_json_for_dynamic_presentation(input_json_path, output_json_path):
#     """Adapt JSON for dynamic presentation - same interface as app.py"""
#     return JSONAdapter.adapt_json_for_ppt(input_json_path, output_json_path)

# # MAIN EXECUTION (same structure as app.py)
# def main():
#     # First, adapt the JSON structure
#     input_json = "presentation_with_images_optimized.json"
#     adapted_json = "adapted_presentation.json"
    
#     # Adapt the JSON structure
#     adapt_json_for_dynamic_presentation(input_json, adapted_json)
    
#     # Generate presentation with dynamic layouts
#     output_path = generate_dynamic_presentation(adapted_json, "Dynamic_Layout_Presentation.pptx")
    
#     print(f"\n🎨 Dynamic Layout Presentation Generated: {output_path}")
#     print("📊 Layout Strategy Applied:")
#     print("   • 0 images → Text-only centered layout")
#     print("   • 1 image  → Single image on LEFT, text on RIGHT")
#     print("   • 2+ images → Double images on RIGHT, text on LEFT")
...
class DynamicLayoutPresentationGenerator:
    ...
    def create_modern_title_slide(self):
        ...
        # Main title - FIXED: Better positioning
        ...
        for paragraph in title_frame.paragraphs:
            paragraph.font.size = Pt(36)  # Reduced from 44
            paragraph.font.color.rgb = self.colors['text_primary']
            paragraph.font.bold = True
            paragraph.font.name = "Segoe UI"
            paragraph.alignment = PP_ALIGN.CENTER
        ...
        for paragraph in subtitle_frame.paragraphs:
            paragraph.font.size = Pt(18)  # Reduced from 20
            paragraph.font.color.rgb = self.colors['primary']
            paragraph.font.name = "Segoe UI"
            paragraph.alignment = PP_ALIGN.CENTER
    ...
    def create_text_only_slide(self, slide_data):
        """Layout with only text content (centered) - FIXED: Centered text"""
        slide = self.prs.slides.add_slide(self.prs.slide_layouts[6])
    ...
    def create_single_image_slide(self, slide_data, image_data):
        """Single image on LEFT side, text on RIGHT - FIXED: Centered text and boundaries"""
        slide = self.prs.slides.add_slide(self.prs.slide_layouts[6])
        ...
        # Image Panel (LEFT) - 40% width (reduced from 45%)
        ...
        # Text Panel (RIGHT) - 55% width (increased from 50%) - FIXED: Proper boundaries
        ...
        # Add image to LEFT panel - FIXED: Smaller image
        image_width = image_panel_width - Inches(1.2)  # Reduced from 1.0
        image_height = Inches(3.0)  # Reduced from 3.5
        image_left = Inches(0.5) + (image_panel_width - image_width) / 2
        image_top = Inches(2.0)  # Adjusted position
    ...
    def create_double_images_slide(self, slide_data, image_data_list):
        """Double images on RIGHT side, text on LEFT - FIXED: Centered text and boundaries"""
        slide = self.prs.slides.add_slide(self.prs.slide_layouts[6])
        ...
        # Text Panel (LEFT) - 55% width (increased from 50%) - FIXED: Proper boundaries
        text_panel_width = self.slide_width * 0.55
        text_panel = slide.shapes.add_shape(
            MSO_SHAPE.RECTANGLE,
            Inches(0.5), Inches(1.3),
        ...
        # Images Panel (RIGHT) - 40% width (reduced from 45%)
        ...
        # Add two images to RIGHT panel (stacked vertically) - FIXED: Smaller images
        image_width = Inches(3.5)  # Reduced from 4.0
        image_height = Inches(1.8)  # Reduced from 2.2
        image_left = images_panel_left + (images_panel_width - image_width) / 2
        ...
        # Add second image (BOTTOM)
        image2_top = image1_top + image_height + Inches(0.4)  # Reduced gap
        try:
            img2 = slide.shapes.add_picture(
                image_data_list[1]["image_path"],
                image_left, image2_top,
    ...
    def add_image_placeholder(self, slide, left, top, width, height):
        ...
        text_frame.paragraphs[0].alignment = PP_ALIGN.CENTER
        text_frame.paragraphs[0].font.color.rgb = self.colors['text_secondary']
        text_frame.paragraphs[0].font.size = Pt(12)  # Reduced size
        text_frame.paragraphs[0].font.name = "Segoe UI"
    
    # REMOVED: add_image_caption method completely
    ...
    def style_slide_title(self, title_frame):
        """Style slide titles"""
        for paragraph in title_frame.paragraphs:
            paragraph.font.size = Pt(24)  # Reduced from 28
            paragraph.font.bold = True
            paragraph.font.color.rgb = self.colors['text_primary']
            paragraph.font.name = "Segoe UI"
            paragraph.alignment = PP_ALIGN.CENTER
