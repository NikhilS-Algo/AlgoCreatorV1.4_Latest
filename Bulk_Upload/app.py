...
def process_bulk_pptx_files_with_progress(user_id: str, input_folder: str, pptx_files: List[str], tracker: BulkUploadProgressTracker):
    ...
    class BulkCorpusBuilderWithProgress:
        ...
        def _step5_create_embeddings(self, step4_input_path: str):
            """Create embeddings - WITH UNIQUE ID GENERATION"""
            ...
            try:
                ...
                if documents:
                    ...
                    # 🚨 CRITICAL: Generate FRESH unique IDs for THIS bulk upload
                    import uuid
                    for i, doc in enumerate(documents):
                        # Create session-specific unique ID
                        original_id = doc['id']
                        new_id = f"bulk_{self.session_id}_{uuid.uuid4().hex[:8]}_{i}"
                        doc['id'] = new_id
                        print(f"   🔄 ID: {original_id} → {new_id}")
                    
                    self.tracker.update_step(self.tracker.Step.EMBEDDING, 80, "Storing embeddings...")
                    
                    # Store with fresh IDs
                    ...
                    print(f"✅ Stored {len(documents)} documents with FRESH IDs in vector database")
                else:
        ...
        def _cleanup_old_data_before_start(self):
            """Clean up any leftover files from previous runs - SIMPLIFIED"""
        ...
        def _append_chromadb_to_generation(self):
            """Append ChromaDB data - SIMPLIFIED"""
            ...
            # If no ChromaDB exists, skip
            if not os.path.exists(bulk_chroma_path):
                print(f"⚠️ No ChromaDB found at: {bulk_chroma_path}")
                return 0
            ...
            try:
                ...
                # Simple test first
                try:
                    doc_count = bulk_collection.count()
                    print(f"📊 Found {doc_count} documents")
                except Exception as test_error:
                    print(f"⚠️ ChromaDB test failed: {test_error}")
                    return 0
                ...
                if not bulk_data['ids']:
                    print("⚠️ No documents to append")
                    return 0
                
                print(f"📊 Will append {len(bulk_data['ids'])} documents")
                ...
                # Generate NEW unique IDs for generation (avoid any conflicts)
                import uuid
                unique_ids = [f"gen_{uuid.uuid4().hex[:16]}" for _ in bulk_data['ids']]
                ...
                print(f"✅ Appended {added_count} documents to generation")
                return added_count
