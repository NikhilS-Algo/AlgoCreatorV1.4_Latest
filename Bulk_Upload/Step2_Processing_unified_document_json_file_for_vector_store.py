...
class DocumentPreprocessor:
    ...
    def repair_text_chunk(self, text: str):
        """Repair and clean text chunk, return None if unfixable"""
        if not text or not isinstance(text, str):
            return None
        
        import re
        
        # 1. Remove invalid Unicode characters (FIX for problem #4)
        try:
            # Try to encode/decode to catch invalid Unicode
        ...
        # 5. Check if empty (FIX for problem #1)
        stripped = text.strip()
        if not stripped:
            return None
        
        # 6. Check for special chars only (FIX for problem #2)
        ...
        # 8. Check length - TRUNCATE if too long (FIX for problem #3)
        MAX_CHARS = 8000  # ~2000 tokens, safe for OpenAI
        if len(stripped) > MAX_CHARS:
            print(f"⚠️ Truncating long text from {len(stripped)} to {MAX_CHARS} chars")
            # Smart truncation at sentence boundary
            if '.' in stripped[:MAX_CHARS]:
                last_period = stripped[:MAX_CHARS].rfind('.')
    ...
    def split_by_sentences(self, text: str, max_words: int = 50) -> List[str]:
        ...
        for sentence in sentences:
            # REPAIR each sentence first
            repaired_sentence = self.repair_text_chunk(sentence)
...
class PPTXProcessor(DocumentPreprocessor):
    def process_slide(self, slide_data: Dict) -> List[Dict]:
        ...
        raw_text = self.merge_text_elements(slide_data['text'])
        text = self.repair_text_chunk(raw_text)
        images = slide_data.get('images', [])
        if text is None:
            print(f"⚠️ Skipping slide {slide_index} in {doc_id} - no meaningful content")
            return []
        ...
        if len(bullet_sections) > 1:
            ...
            for section in bullet_sections:
                ...
                if current_word_count + section_word_count > 50 and current_chunk_text:
                    chunks.append({
        ...
        text_chunks = self.split_by_sentences(text, max_words=50)
        chunks = []
